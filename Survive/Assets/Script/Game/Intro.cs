        ...
        MethodMouse[] m = new MethodMouse[]
{
    ...
    void loadSetting()
    {
        ...
        for (int i = 0; i < 4; i++)
        {
            ...
            //for (int j = 0; j < 2; j++)
            {
                ...
                {
                    st.setString(i + "\n" + "<" + "\n");
                    img.position = new iPoint(50, 150);
                }
                else if (i == 1)
                {
                    st.setString(i + "\n" + ">" + "\n");
                    img.position = new iPoint(400, 150);
                }
                else if (i == 2)
                {
                    st.setString(i + "\n" + "<" + "\n");
                    img.position = new iPoint(50, 350);
                }
                else if (i == 3)
                {
                    st.setString(i + "\n" + ">" + "\n");
                    img.position = new iPoint(400, 350);
                }
                img.add(st.tex);
            }
            pop.add(img);
    ...
    void methodStSetting(iStrTex st)
    {
        ...
        drawString(SoundManager.instance().printVolume(iSound.BGM), new iPoint(st.tex.tex.width / 2, 150), TOP | HCENTER);

        drawString("Effect Volume", new iPoint(st.tex.tex.width / 2, 250), TOP | HCENTER);
        drawString(SoundManager.instance().printVolume(iSound.ButtonClick), new iPoint(st.tex.tex.width / 2, 350), TOP | HCENTER);
    }
